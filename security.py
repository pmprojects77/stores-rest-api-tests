...
def authenticate(username, password):
    """
    Fuunction that gets called when a user call the /auth endpoint
    with their username and password

    :param username: User's username is string format
    :param password: User's un-encrypted password in string format
    :return: A UserModel if auth successful, None otherwise.
    """
...
def identity(payload):
    ...
    verified their authorization herader is correct
